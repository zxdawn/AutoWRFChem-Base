...
def make_era_ml_grib_list(start_date, end_date):
...
def make_era_pl_grib_list(start_date, end_date):
    if (type(start_date) is not dt.datetime and type(start_date) is not dt.date or
            type(end_date) is not dt.datetime and type(end_date) is not dt.date):
        raise TypeError("start_date and end_date must be datetime or date objects")

    era_files = []
    era_files += list_era_pl_grib_files(start_date, end_date, 'pl')
    era_files += list_era_pl_grib_files(start_date, end_date, 'sl')

    return era_files

def list_era_ml_grib_files(start_date, end_date, type):
...
def list_era_pl_grib_files(start_date, end_date, type):
    # ERA5 files are daily
    start_date = start_date.replace(minute=0, second=0, microsecond=0)

    file_pattern = 'ecmf_{date}_{type}.grib'
    files = []
    curr_date = start_date
    while curr_date <= end_date:
        files.append(file_pattern.format(date=curr_date.strftime('%Y%m%d'), type=type))
...
def parse_args():
    allowed_mets = ["narr","era-interim.ml","era-interim.pl"]
...
if __name__ == "__main__":
    ...
    elif met == "era-interim.ml":
        if do_grib:
            files = make_era_ml_grib_list(start_datetime, end_datetime)
        else:
            raise RuntimeError("Met type {0} only supports grib format".format(met))
        print(delim.join(files))

    elif met == "era-interim.pl":
        #if do_grib:
        files = make_era_pl_grib_list(start_datetime, end_datetime)
        #else:
        #    raise RuntimeError("Met type {0} only supports grib format".format(met))
        print(delim.join(files))
